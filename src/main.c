...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
    ...
    *appstate = malloc(sizeof(AppState));
    ...
    renderer->window = SDL_CreateWindow(renderer->title, renderer->width, renderer->height, 0 | SDL_WINDOW_VULKAN);
    if (!renderer->window) {
        SDL_Log("Couldn't initialize SDL: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }

    renderer->gpu = SDL_CreateGPUDevice(SDL_GPU_SHADERFORMAT_SPIRV, false, NULL);
    if (!renderer->gpu) {
        SDL_Log("Couldn't initialize SDL: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }
...
SDL_AppResult SDL_AppIterate(void *appstate) {
    ...
    // bool success = SDL_WaitAndAcquireGPUSwapchainTexture(renderer->command_buffer, renderer->window);
